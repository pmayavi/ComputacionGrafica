...
public class Plane implements IntersectableObject {
    ...
    public Solution intersect(Ray ray) {
        ...
        double numerador = -(Vector4.dotProduct(ray.origin, n) + d);
        // Calcula el denominador de la ecuación de la intersección
        double denominador = Vector4.dotProduct(ray.direction, n);
        ...
        Vector4 puntoInterseccion = Vector4.add(ray.origin, Vector4.multiply(ray.direction, t));

        // Crea una instancia de Solution para almacenar la info de la intersección
        Solution sol = new Solution(puntoInterseccion, n, Scene.colors.get(colorIndex),
